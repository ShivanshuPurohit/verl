...
To run verl locally, pull the docker image from our azure registry:

```bash
docker pull synthlabstest.azurecr.io/verl/verl-fsdp:v3
docker run -it --gpus all --mount type=bind,src=/path/to/your/verl,dst=/verl --shm-size=10gb --env-file=.env
```

where `.env` is an environment file containing the `HF_TOKEN` and `WANDB_API_KEY` api keys.

### Running on tracto

To run on tracto, you first need to set up the tracto CLI to autheticate your local machine. 

#### Tracto CLI
Install the `yt` and [`tractorun`](https://github.com/tractoai/tractorun) CLIs.
```
pip install ytsaurus-client ytsaurus-yson
```

When setting up `yt`, you'll need to create a `~/.yt/config` file, which is
```
cat ~/.yt/config.yaml
"proxy"={
  "url"="<your-yt-proxy-here>";
};
"token"="<your-yt-token-here>
```
(note: the config file is a yson format and has no file extension)

Once done, make your training directory on tracto.
```
yt create map_node --path "//home/<your-username>/gptx-training-dir" --ignore-existing
```
...
```
uv venv
uv pip install -U "tractoray[ray]"
uv run tractoray start --docker-image cr.eu-north1.nebius.cloud/e00faee7vas5hpsh3s/chiffa/synthlabs_tractoray2:v2 --workdir //home/path/to/your/ray/workdir --node-count=2 --pool-trees=default_gpu_h100 --gpu-limit 8 --memory-limit 1073741824000
```

This will install `tractoray` and then launch a cluster with two 8xH100 nodes with 1TB of RAM each.

The command will output a `RAY_ADDRESS` and `RAY_JOB_HEADERS` which you should `export` as environment variables.

You can then submit a job 
```
cd verl
ray job submit --no-wait --working-dir . -- bash examples/to/your/script.sh
```
...
```bash
docker build -f verl/docker/Dockerfile.fsdp -t verl .   
```

### Common Bugs 

* A list index out of range error with a traceback pointing to the device mesh placement group, this is an error with the docker container and you should kill and restart it.
```
File "/usr/local/lib/python3.12/dist-packages/torch/distributed/device_mesh.py", line 721, in get_group                                                                                    
_find_pg_by_ranks_and_tag(*self._dim_group_infos[mesh_dim][:2])  # type: ignore[index] 
```

* A segfault error `ncclncclTopoCheckNet` means something is wrong with your local installation. If that occurs then you should run in a docker container instead.

* If wandb says it can't connect/log in - make sure you pass in your environment file via `trainer.env` and that is contains your wandb api key.
